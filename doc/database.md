...
- `user_managers` links users to one or more responsible admin users. The responsible user must be validated by backend policy as an approved admin. This table is treated as a hierarchy: admins can be assigned under other admins, and backend review queries walk the graph recursively so higher-level admins can manage every direct and indirect descendant. Cycles are rejected by the admin assignment API.
- `administrative_requests` stores Vacation, Leave, Smart working, and historical activity-change requests. Active requests cannot overlap other requests for the same user; backend policy also prevents overlaps with work sessions and prevents work sessions during approved non-activity requests. Request review follows the recursive `user_managers` hierarchy. Top-level admins with no assigned responsible can self-review; nested admins cannot self-review and must be reviewed by one of their ancestors or by root. Pending requests are hard-deleted by their owner, while already reviewed requests use `deleted_at` so the original approval/revocation status remains visible. Historical activity changes use `activity_change_action` and `activity_change_payload`; they are applied to `time_sessions`, `session_tags`, and `time_events` only when approved. Root users can clean only requests that fully ended before the current month.
- `administrative_request_history` stores per-viewer archival state for already reviewed administrative requests. Moving a request to history does not mutate the canonical request row; it hides the request from the current viewer's primary lists and exposes it in the history modal. `removed_at` removes it from that viewer's history without restoring it to the primary lists. When an admin clears their history, the backend also creates/removes the corresponding history visibility for the involved requester.
- `time_sessions` stores clocked work intervals. It enforces one open session per user and rejects overlapping sessions for the same user. `no_count_minutes` stores minutes excluded from effective report and overtime totals.
...
- `tags` stores user-owned default and custom tags. `Presence`, `Smart working`, and `Not billable` are the protected default tags created for each user; additional tags are user-created custom tags. `default_tag_templates` seeds the protected defaults with colors for registration-time tag creation; all tag colors are constrained to six-digit hex values.
...
erDiagram
    ...
    users ||--o{ administrative_request_history : views_history
