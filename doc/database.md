...
- `administrative_requests` stores Vacation, Leave, and Smart working requests. Active requests cannot overlap other requests for the same user; backend policy also prevents overlaps with work sessions and prevents work sessions during approved requests. Pending requests are hard-deleted by their owner, while already reviewed requests use `deleted_at` so the original approval/revocation status remains visible. Root users can clean only requests that fully ended before the current month.
