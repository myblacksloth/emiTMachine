...
- `administrative_requests` stores Vacation, Leave, and Smart working requests. Active requests cannot overlap other requests for the same user; backend policy also prevents overlaps with work sessions and prevents work sessions during approved requests. Request review follows the recursive `user_managers` hierarchy. Top-level admins with no assigned responsible can self-review; nested admins cannot self-review and must be reviewed by one of their ancestors or by root. Pending requests are hard-deleted by their owner, while already reviewed requests use `deleted_at` so the original approval/revocation status remains visible. Root users can clean only requests that fully ended before the current month.
