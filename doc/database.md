...
- `users` stores multi-user accounts with role, status, timezone, password hash metadata, and a mutable `public_id`. The internal `id` remains the relational primary key; `public_id` is the admin-editable user-facing identifier generated as a UUID by default.
